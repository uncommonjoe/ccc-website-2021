...
angular.module("fitVids", []).directive("responsiveVideo", [
	"$timeout",
	function ($timeout) {
		"use strict";
			...
			link: function (scope, element, attr) {
				...
				$timeout(function () {
					...
					angular.forEach(videos, function (item) {
						...
						if (
							(item.tagName.toLowerCase() === "embed" &&
								$item.parent().tagName === "object" &&
								$item.parent().length) ||
							$item
								.parent()
						...
						height =
							item.tagName.toLowerCase() === "object" ||
							$item.attr("height")
								? parseInt($item.attr("height"), 10)
								: $item.height();
						width = !isNaN(parseInt($item.attr("width"), 10))
							? parseInt($item.attr("width"), 10)
							: $item.width();
						aspectRatio = height / width;

						if (!$item.attr("id")) {
							var videoID =
								"fitvid" + Math.floor(Math.random() * 999999);
